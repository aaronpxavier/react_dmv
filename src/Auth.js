...
export function getToken4() {
  return function action(dispatch) {
    // dispatch({ type: _getTokenStarted() });
    const req = _createMSAL().loginPopup();

    return req.then(
      //const { idToken } = res;
      (res) => dispatch({ type: _getTokenSuccess(res) }),
      (error) => dispatch({ type: _getTokenFailed(error) })
    );
  };
}
